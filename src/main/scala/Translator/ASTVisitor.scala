...
class ASTVisitor(val debug: Boolean = false) {
  ...
  def visitProgram(ctx: ProgramContext): Unit = {
    ctx.module_def().actor_def().asScala.foreach { actorCtx =>
      ...
      actorProcedures(name)(endId) = IREnd(endId)
    }
    if (debug) dumpIR()
  }
  ...
  private def translateStatement(ctx: StatementContext, actor: String, currentPc: Int): Int = {
    ...
    if (ctx.send_stmt() != null) {
      ...
      val sendInstr = IRQueuePush(currentPc, nextPc, qName, s.msg_name().getText)
    ...
    // RECEIVE
      ...
      val receiveInstr = IRQueuePop(currentPc, nextPc, qName, r.msg_name().getText)
    ...
    // RECEIVE ALTS
      ...
      val cases = rAlts.receive_case().asScala.map { c =>
        ...
        val jumpInstr = IRJump(bodyEnd, exitPc)
      ...
      val branchInstr = IRBranch(currentPc, cases, None)
    ...
    // CHOOSE
      ...
      val choiceInstr = IRChoice(currentPc, branchStarts)
      ...
      branches.zip(branchStarts).foreach { (branch, start) =>
        ...
        val jumpInstr = IRJump(end, afterChoice)
    ...
    // REPEAT
      ...
      if (r.NUMBER() != null) {
        ...
        // Uncountable loop
        val bodyEnd = translateBlock(r.block(), actor, loopStart)
        val jumpInstr = IRJump(bodyEnd, loopStart)
        actorProcedures.getOrElseUpdate(actor, mutable.Map.empty[Int, IRInstruction]).update(bodyEnd, jumpInstr)
      }
    ...
    // BREAK
      ...
      if (breakStack.nonEmpty) {
        val jumpInstr = IRJump(currentPc, breakStack.top)
    ...
    // PARALLEL
      ...
      val parallelExecInstr = IRParallelExec(currentPc, branchStarts, joinPc)
      ...
      branches.zip(branchStarts).foreach { (b, start) =>
        ...
        val parallelEndInstr = IRParallelEnd(end, joinPc) // Wait here for all parallel branches to end
  ...
  private def formatInstr(i: IRInstruction): String = i match
    ...
    case IRParallelExec(_, branches, breakExit) => s"PARALLEL START: branches -> [${branches.mkString(", ")}], break -> [$breakExit]"
    case IRParallelEnd(_, j) => s"PARALLEL_END"
    case IRQueuePush(_, next, q, m) => s"PUSH $m to $q"
